...
def run_epoch(model, dataloader, epoch, optimizer=None, scheduler=None):
    ...
    except StopIteration: raise RuntimeError("empty model. ")
...
def finetune(args: DictConfig) -> None:
    ...
    train_set = eval(args.dataset.upper())(root=data_dir, train=True, transform=train_transform, download=False)
    test_set = eval(args.dataset.upper())(root=data_dir, train=False, transform=test_transform, download=False)
    ...
    optimal_loss, optimal_acc = 100, 0.
